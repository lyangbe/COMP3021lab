...
public class NoteBookWindow extends Application {
	...
	private HBox addHBox() {
		...
		hbox.getChildren().addAll(buttonLoad, buttonSave, lb_search, txt_search, btn_search, btn_clear);
	...
	private VBox addVBox() {
		...
		for(int i = 0; i<f.size();i++) {
			foldersComboBox.getItems().add(f.get(i).getName());
		}
	...
	private void updateListView() {
        ...
        List<Folder> f = noteBook.getFolders();
        List<Note> notes = null;
        if (currentFolder != null) {
        	for(Folder fod : f) {
            	if(fod.getName().equals(currentFolder)) {
            		if (!currentSearch.equals("")){
            			notes= fod.searchNotes(currentSearch);
            		}else {
            			notes = fod.getNotes();
                    }
            		if(notes != null) {

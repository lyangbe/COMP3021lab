...
public class HeapTest {
    ...
    public static void testPoll() {
        output("Test Poll");
        Heap<Integer> integerHeap = new Heap<>();
        List<Integer> values = Arrays.asList(2, 53, 213, 5, 1, 5, 4, 210, 14, 26, 
44, 35, 31, 33, 19, 52, 27);
        integerHeap.addAll(values);
        Collections.sort(values);
        for (int x : values) {
        	//output("target: "+Integer.valueOf(x));
        	//output("current: "+integerHeap.poll());
    ...
    public static void testSize() {
        output("Test Size");
        Heap<Integer> integerHeap = new Heap<>();
        List<Integer> values = Arrays.asList(10, 14, 26, 44, 35, 31, 33, 19, 52, 
27);
        integerHeap.addAll(values);
